...
- [ESLint Config Generator](https://draftkit.co/dev-launch-pack/eslint-config-generator.html) - Free browser-based tool that generates a ready-to-commit ESLint config (flat `eslint.config.js` for ESLint 9+ or legacy `.eslintrc` for ESLint 8) from your chosen language, framework, rules, and environment.
